...
function validateRecaptcha()
{
    ...
    if (isset($post['name']) && isset($post['hire']) && isset($post['description']) && isset($post['recaptcha']) && isset($post['emailAddress'])) {
...
{
    $to   =  BUSSINESS_MAIL_TO;
    $from   =  BUSSINESS_MAIL_FROM;
    ...
    $replace = array('{name}', '{email}', '{companyName}', '{hire}', '{contact}', '{description}');
    $with = array($data['name'], $data['emailAddress'], (isset($data['companyName']) ? $data['companyName'] : "-"), $data['hire'], (isset($data['contact']) ? $data['contact'] : "-"), $data['description']);
    $contents = file_get_contents('businessInternal.html');
    ...
    if ($success) {
        ...
        echo json_encode($replyObject);
        $replace = array('{name}');
        $with = array($data['name']);
        $contents = file_get_contents('businessClient.html');
